...
const logger = new Logger('Firefox');
...
export class BaseRecvHandler extends Handler
{
	constructor(rtpParametersByKind, settings)
	{
		super('recv', rtpParametersByKind, settings);

		if (this.constructor.name === BaseRecvHandler.name)
		{
			throw new TypeError('Cannot construct BaseRecvHandler instances directly');
		}

		if (this._getRemoteTrack === undefined)
		{
			throw new TypeError('Must override method _getRemoteTrack');
		}
	...
	addConsumer(consumer)
	{
		...
		return Promise.resolve()
			...
			.then(() =>
			{
				return this._getRemoteTrack(consumerInfo);
			});
	}
...
export function getNativeRtpCapabilities()
{
