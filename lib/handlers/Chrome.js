...
export class BaseSendHandler extends Handler
{
	constructor(rtpParametersByKind, settings)
	{
		super('send', rtpParametersByKind, settings);

		if (this.constructor.name === BaseSendHandler.name)
		{
			throw new TypeError('Cannot construct BaseSendHandler instances directly');
		}

		if (this.replaceProducerTrack === undefined)
		{
			throw new TypeError('Must override method replaceProducerTrack');
		}
